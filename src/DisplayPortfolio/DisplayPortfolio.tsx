...
const DisplayPortfolio = (props: Props) => {
  ...
  const imgUrl = "https://jarvis-portfolio.s3.amazonaws.com/profileImg_" + localStorage.getItem("userId");
...
//  const [gitHubID, setGitHubID] = useState('');
 ...
 // const [gitHubLink, setGitHubLink] = useState('');
  ...
  const fetchPersonalDetails = async (e: SyntheticEvent<HTMLDivElement>) => {
    ...
    try {
      const response = await axios.get(
        "http://localhost:8000/api/personaldetails/" + userId
      );
  ...
  const fetchSkills = async () => {
    ...
    try {
      const response = await axios.get(
        "http://localhost:8000/api/getskillsbyid/" + userId
      );
      if (response.status == 200 && response.data) {
        console.log(response.data)
        var skills: any = [];
  ...
  const fetchExperience = async () => {
    ...
    try {
      const response = await axios.get(
        "http://localhost:8000/api/getexperiencebyid/" + userId
      );
      if (response.status == 200 && response.data) {
 ...
 const fetchContact = async () => {
        ...
        try {
          const response = await axios.get(
            "http://localhost:8000/api/contact/" + userId
          );
          if (response.status == 200 && response.data) {
...
const fetchProjectWork = async () => {
        ...
        try {
          const response = await axios.get(
            "http://localhost:8000/api/projectWork/" + userId
          );
          if (response.status == 200 && response.data) {
    ...
    <div className="display-page">
      ...
      <div className="container" onLoad={fetchProjectWork}>
                    ...
                    <div className="row">
                        ...
                        {workProject.map((we : { proName: any , proDesc: any, tech: any,gitHubLink:any,webLink:any }) => (

                            <div className="icon-box">
                                <h4 className="title">Project Name :{we.proName}</h4>
                                <p className="description">Description:{we.proDesc}</p>
                                <div className="project-stack">
                                  <li className="project-stack-item">Technology: {we.tech}</li>
                                  <li className="project-stack-item">GitHub Link:{we.gitHubLink}</li>
                                  <li className="project-stack-item">Web Link:{we.webLink}</li>
                                </div>
                                <a href='#'> <LinkIcon sx={{ color: "black" }} >add_circle</LinkIcon></a>
                                <a href='#'> <GitHubIcon sx={{ color: "black" }}></GitHubIcon></a>
                            </div>
                                 ))}
                        </div>
          ...
          <div className="section-title">
            ...
            <div className="work-exp experience section-padding">
              <div className="row">
                ...
                {workExp.map((we : { company: any , jobtitle: any, jobstatus: any }) => (
                  <div className="experience-wrapper">
                    <div className="company-wrapper clearfix">
                      <div className="experience-title">{we.company}</div>
                      <div className="time">{we.jobstatus}</div>
                    ...
                    <div className="job-wrapper clearfix">
                      <div className="experience-title">SDE 1 Engineer </div>
                      <div className="company-description">
                        <p>Vancouver</p>
                      </div>
                      <div className="company-description">
                ...
                <div className="container" onLoad={fetchContact}>
                    ...
                    <div className="row">
                                ...
                                <div className="social-links">
                                    <a href="#" className="gitHub"><GitHubIcon></GitHubIcon></a>
                                    <a href="#" className="linkedin"><LinkedInIcon></LinkedInIcon></a>
                                    <a href="#" className="outlock"><EmailIcon></EmailIcon></a>
                                </div>
                                {workContact.map((we : { email: any , phone: any, tech: any, linkID:any, gitHubID:any }) => (

                        <div className="col-lg-6 col-md-6">
                            <div className="info">

                                <div className="d-flex align-items-center mt-4 contact-info">
                                    <EmailIcon className='icon-logo'></EmailIcon><p>{we.email}</p>
                                </div>
                                <div className="d-flex align-items-center mt-4 contact-info">
                                    <PhoneIcon className='icon-logo'></PhoneIcon><p>{we.phone}</p>
                                </div>
                                <div className="d-flex align-items-center contact-info">
                                <LinkedInIcon className='icon-logo'></LinkedInIcon><p>{we.linkID}</p>
                                </div>
                                <div className="d-flex align-items-center contact-info">
                                <GitHubIcon className='icon-logo'></GitHubIcon><p>{we.gitHubID}</p>
                                <a href="#" className="linkedin"><LinkedInIcon></LinkedInIcon></a>
                                </div>
